...
def assert_performance_budget(
    ...
    if current["p95_operation_seconds"] > allowed_p95:
        ratio = float(current["p95_operation_seconds"]) / float(baseline["p95_operation_seconds"])
        raise ValueError(
            ...
            f"ratio={ratio:.3f}, allowed={allowed_p95:.6g}s)"
        )
...
def main() -> int:
    ...
    if arguments.macos_ci_evidence_output:
        if not all((arguments.runner_os, arguments.python_version, arguments.uv_version)):
            parser.error(
                "--macos-ci-evidence-output requires --runner-os, "
                "--python-version, and --uv-version"
            )
    ...
    if arguments.windows_ci_evidence_output:
        if not all((arguments.runner_os, arguments.python_version, arguments.uv_version)):
            parser.error(
                "--windows-ci-evidence-output requires --runner-os, "
                "--python-version, and --uv-version"
            )
